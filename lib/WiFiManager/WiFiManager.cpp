...
void WiFiManager::setupConfigPortal() {
  ...
  server->on(String(FPSTR(R_root)).c_str(),       std::bind(&WiFiManager::handleWifi, this, true));
  ...
  server->on(String(FPSTR(R_scan)).c_str(),       std::bind(&WiFiManager::handleScan, this));
  server->on(String(FPSTR(R_state)).c_str(),       std::bind(&WiFiManager::handleState, this));
  server->on(String(FPSTR(R_id)).c_str(),         std::bind(&WiFiManager::handleId, this));
...
// zarko.runjevac 15.5.2019
/** Handle the scan page */
void WiFiManager::handleScan() {
  DEBUG_WM(DEBUG_VERBOSE,F("State - json"));
  handleRequest();
...
void WiFiManager::handleId(){
  DEBUG_WM(DEBUG_VERBOSE,F("ID - json"));
  handleRequest();
  server->sendHeader("Cache-Control", "no-cache, no-store, must-revalidate");
  server->sendHeader("Pragma", "no-cache");
  server->sendHeader("Expires", "-1");
